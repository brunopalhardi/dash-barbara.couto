...
describe("filtro de período em fuso America/Sao_Paulo", () => {
  // Janela do dia 02/06/2030 no calendário BR.
  const RANGE = { from: "2030-06-02", to: "2030-06-02" };

  beforeAll(async () => {
    await db.delete(purchases).where(eq(purchases.transactionId, "TZ-PREV"));
    await db.delete(purchases).where(eq(purchases.transactionId, "TZ-SAME"));
    await db.insert(purchases).values([
      {
        ...
        valueCents: 29700,
        purchasedAt: new Date("2030-06-02T02:00:00Z"),
        rawPayload: {},
      },
      {
        ...
        valueCents: 29700,
        purchasedAt: new Date("2030-06-02T12:00:00Z"),
        rawPayload: {},
      },
    ]);
  });
